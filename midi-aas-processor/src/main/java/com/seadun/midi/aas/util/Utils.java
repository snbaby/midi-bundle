...
import com.seadun.midi.aas.VerticleApplication;
...
public class Utils {
	...
	public static void loadYml() throws FileNotFoundException {
		Yaml yaml = new Yaml();
		String ymlPath = System.getProperty("user.dir") + System.getProperty("file.separator")
				+ "application-server.yml";
		String ymlPathOther = System.getProperty("user.dir") + System.getProperty("file.separator")
		...
		if (fOtherFile.exists()) {
			yaml.loadAs(new FileInputStream(fOtherFile), Application.class);
		...
		log.info("yml 配置加载init成功----->" + JSON.toJSONString(Application.init));
	}
}
